      ...
      column(1,
       ...
       fluidRow(
          column(2,offset = 8,
             ...
             sliderInput(inputId = ns("slider_width"),min = 15,max = 75,value = 30,label=""))))
...
M0_Hist <- function(input,output,session,Vals){
  ...
  Reac_Hg <- eventReactive(input$Hg_Ac, {
    ...
    out <- list(p=p,gg=gg,group_names=group_names,gr_names=Gr_names,color_count = color_count ,cc1 = cc1)
...
React_DT3 <-eventReactive(input$DT_AC3, { 
  ...
  gg_hist <- ggplotly(gg_hist0) %>% layout(annotations = text_hist) 
  
  
  
  out <- list(gg_hist0=gg_hist0,gg_bar=gg_bar,gg_hist=gg_hist,group_names=group_names,gr_names=Gr_names,color_count = color_count ,cc1 = cc1,c=c,gr_num=gr_num)
  ...
  React_GrCat_final <- reactive({
    ...
    if(input$density_r){
      ...
      p <- React_DT3()$gg_hist0 + geom_density(aes(y=100*..density..),alpha=0.3, fill="#9B2335",colour="#9B2335",lwd=1.5)+
        ...
        A <- subplot(React_DT3()$gg_bar,p,shareX = FALSE,shareY = FALSE,
                     widths=c(input$slider_width/100,1-input$slider_width/100),
                     titleX=TRUE) %>% layout(showlegend = FALSE)
        return(A)
