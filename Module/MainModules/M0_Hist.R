...
M0_Hist <- function(input,output,session,Vals,format_out,font_plot){
  ...
  # 
  # ch_bin <- reactive({
  #   return(length(unique(group_mean()[,1])))
  #   })
...
#  output$Hg_2 <- renderPlotly(React_GrCat_final())
  ...
  output$download <- downloadHandler(
    ...
    filename = function(){
      paste('گزارش گروه بندی', sep = '.', switch(format_out(),HTML = 'html', PDF = 'pdf', Word = 'docx'))
    },
    content=function(file){
      withProgress(message = "... گزارش در حال ساخته شدن است",
                   min = 0,max = 100,value = 72, {
                     ...
                     params <- list(n = Reac_Hg_final()$A,m= React_out_table_l())
